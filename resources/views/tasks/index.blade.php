    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <form action="{{ route('tasks.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            ...
            @if(request('query') || request('category'))
                <a href="{{ route('tasks.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Wyczyść
                </a>
            @endif
        </form>
        ...
        @empty
            <div class="text-center py-20 bg-white rounded-xl border border-gray-100 border-dashed">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    ...
                    @else
                        Rozpocznij tworząc nowe zadania w katalogu storage.
                    @endif
                </p>
                @if(request('query') || request('category'))
                    <div class="mt-6">
